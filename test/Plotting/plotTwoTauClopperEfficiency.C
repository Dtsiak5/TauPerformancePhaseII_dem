...
void plotTwoTauClopperEfficiency(){
  ...
  TString fileName1 = "../new_store/TwoTaus_ggH_PU200_PUPPI.root";//Demetra.root"; // /afs/cern.ch/work/d/dtsiakko/private/CMSSW_9_4_2_tauAtMiniAOD/src/RecoTauTag/Configuration/test/RelValZTT_MiniAOD_PU200.root";// f_Aug29_hadd/RelValZTT_MiniAOD_PU200.root";//../store/TwoTaus_Demetra.root"; // /data/uhussain/TwoTausEff_Aug8_hadd/GluHtoTT_PU0.root"; 
  TString fileName = "../new_store/TwoTaus_ggH_PU0_PUPPI.root";//Demetra.root"; // /data/uhussain/TwoTausEff_Aug8_hadd/GluHtoTT_PU200.root"; 
  ...
  TString treePath2 = "cutBased/ModFixedStripTaus";//"cutBased/OrgPFTaus"; //"cutBased/ModFixedStripTaus";
  //int bins = 30;
  //double low  = -1.5;
  //double high = 1.5;

  float binarray[]={0,20,40,60,80,100,120,160,200}; //);//140,160,180,200};
  ...
  TString GenCut= "genTauPt> 20 && abs(genTauEta) < 3.0";// && (dm!=5&&dm!=6 && dm > -1)"; // < 1.5 && (dm!=5&&dm!=6 && dm > -1)";
  
  //TString GenCut1= "genTauPt > 22 && abs(genTauEta)> 2.3 && abs(genTauEta) <4.0 && (dmf!=5&&dmf!=6 && dmf > -1) && (dmf == 10) &&"+z3;
 
  TString RecoCut= "tauPt > 20 && abs(tauEta) < 3.0 && taupfTausDiscriminationByDecayModeFinding==1 && tauChargedIsoPtSum<2.5 &&" + GenCut;//tauChargedIsoPtSum<2.5 && taupfTausDiscriminationByDecayModeFinding>-1 &&" + GenCut; //<1.5 && taupfTausDiscriminationByDecayModeFinding==1 &&" + GenCut;
  ...
  //Style
  TString xaxis = "Gen #tau_{vis} P_{T} (GeV)";
  int markerstyle = 20;
  ...
  TString outFileName = "eff_plot_RelVal_PUPPI_2.5"; // "plot-HTT-PT-pu0-200_olddmf";
  TString legLabel = "Z #rightarrow #tau #tau (RelVal)"; // (1000 fb^{-1})";
 ...
 TFile Histo_output("Histo.root","recreate");
 eff->Write();
 eff_1->Write();
 Histo_output.Close();
}
