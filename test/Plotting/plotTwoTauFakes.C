...
void plotTwoTauFakes(){
  ...
  TString fileName = "../new_store/TwoTaus_QCD_PU0_PUPPI.root";///TwoTausQCD_PU0_Demetra.root" ; ///..????? /data/uhussain/TwoTausEff_Aug8_hadd/DYJetsToLL_PU0.root"; 
  TString fileName1 = "../new_store/TwoTaus_QCD_PU200_PUPPI.root";//../store/TwoTausQCD_PU200_Demetra.root"; //../store/ /data/uhussain/TwoTausEff_Aug8_hadd/DYJetsToLL_PU200.root";
  ...
  TString treePath2 = "cutBased/jetModFixedStripTaus";//jetOrgPFTaus"; //jetModFixedStripTaus";
  //int bins = 30;
  //double low  = -1.5;
  //double high = 1.5;
  ...
  //TString GenCut= "jetPt> 20 && jetPt < 200 && genJetMatch ==1 && abs(jetEta) < 3.0 " ;
  //TString GenCut= "jetPt> 20 && jetPt < 200 && genJetMatch ==1 && abs(jetEta) < 3.0 " ; //&& (dm!=5&&dm!=6 && dm > -1)";
  TString GenCut= "jetPt> 20 && genJetMatch ==1 && abs(jetEta) < 3.0 " ;
...
//&& jetTauMatch==1
  TString RecoCut= "tauPt > 20 && abs(tauEta)< 3.0 && taupfTausDiscriminationByDecayModeFinding==1 &&"  + GenCut; //tauChargedIsoPtSum<2.5  &&  && taupfTausDiscriminationByDecayModeFinding==1 &&" + GenCut;
  ...
  TString outFileName = "FAKE_RATE_PLOT_PUPPI"; //plot-Fakes-PT-pu0-200_olddmf";

  TString legLabel = "jet #rightarrow #tau_{h} (QCDFlat)";
 ...
 /////fourth
  ...
  mg->SetMaximum(1.0);
  mg->SetMinimum(0.00016);
  //mg->SetMaximum(0.02);


  TLegend *leg = new TLegend(.50,.300,0.67,.500,legLabel,"nbNDC");
  //TLegend *leg = new TLegend(.30,.735,0.67,.842,legLabel,"nbNDC");
 ...
 TFile Histo_output("Histo_Fake.root","recreate");
 eff->Write();
 eff_1->Write();
 Histo_output.Close();
